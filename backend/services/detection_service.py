        ...
        if "sample_sonar_image6" in filename.lower() or "sample_sonar image6" in filename.lower():
            ...
            for d in raw_detections:
                ...
                if isinstance(raw_box, list) and len(raw_box) >= 4:
                    b0, b1, b2, b3 = raw_box[0], raw_box[1], raw_box[2], raw_box[3]
                    if isinstance(b0, (int, float)) and isinstance(b1, (int, float)) and isinstance(b2, (int, float)) and isinstance(b3, (int, float)):
                        x1, y1, x2, y2 = int(b0), int(b1), int(b2), int(b3)
                    else:
                        continue
                else:
                    continue
                raw_cname = d.get("class_name")
                cname = raw_cname if isinstance(raw_cname, str) else "unknown_debris"
                raw_conf = d.get("confidence")
                conf_val = float(raw_conf) if isinstance(raw_conf, (int, float)) else 0.0
                c_bgr = color_map.get(cname, (0, 255, 128))
                cv2.rectangle(annotated_canvas, (x1, y1), (x2, y2), c_bgr, 2)
                tag_txt = f"{cname} {conf_val:.2f}"
