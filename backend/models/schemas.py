...
class DetectionItem(BaseModel):
    ...
    class_: Optional[str] = Field(default=None, serialization_alias="class", description="Class alias")
    ...
    bbox_coords: Optional[BoundingBox] = Field(default=None, description="Bounding box dictionary {x, y, width, height}")
    center: CenterPoint = Field(..., description="Target geometric acoustic center")
    area: int = Field(..., description="Bounding box pixel area")
    anomaly_score: float = Field(default=0.0, description="Empirical anomaly score (0.0 to 1.0)")
    classification_source: str = Field(default="detector", description="detector | second_stage | unknown")
    anomaly_type: str = Field(default="KNOWN_OBJECT", description="KNOWN_OBJECT or UNCLASSIFIED_ANOMALY")
